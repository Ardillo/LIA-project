...
function AmtStackCreateService(wsmanStack) {
    ...
    // Perform a WSMAN "CREATE" operations.
    obj.CreateWithXml = function (name, putobj, callback, tag, pri) { obj.wsman.ExecCreateXml(obj.CompleteName(name), execArgumentsToXml(putobj), function (ws, resuri, response, xstatus) { obj.xxup(); callback(obj, name, response, xstatus, tag); }, 0, pri); obj.xxup(); }
    ...
    obj.Exec = function (name, method, args, callback, tag, pri, selectos) { obj.wsman.ExecMethod(obj.CompleteName(name), method, args, function (ws, resuri, response, xstatus) { obj.xxup(); callback(obj, name, obj.CompleteExecResponse(response), xstatus, tag); }, 0, pri, selectos); obj.xxup(); }
	
    // Perform a WSMAN method call operation.
    obj.ExecWithXml = function (name, method, args, callback, tag, pri, selectos) { obj.wsman.ExecMethodXml(obj.CompleteName(name), method, execArgumentsToXml(args), function (ws, resuri, response, xstatus) { obj.xxup(); callback(obj, name, obj.CompleteExecResponse(response), xstatus, tag); }, 0, pri, selectos); obj.xxup(); }
    ...
    obj.EnumStartSink = function (name, response, callback, resuri, status, tag, pri) {
        ...
        if (response == null || response.Header["Method"] != "EnumerateResponse" || !response.Body["EnumerationContext"]) { callback(obj, name, null, 603, tag); obj.EnumDoNext(1); return; }
    ...
    obj.EnumContinueSink = function (name, response, callback, resuri, items, status, tag, pri) {
        if (status != 200) { callback(obj, name, null, status, tag); obj.EnumDoNext(1); return; }
        if (response == null || response.Header["Method"] != "PullResponse") { callback(obj, name, null, 604, tag); obj.EnumDoNext(1); return; }
        ...
        if (response.Body["EnumerationContext"]) {
            var enumctx = response.Body["EnumerationContext"];
            obj.wsman.ExecPull(resuri, enumctx, function (ws, resuri, response, xstatus) { obj.EnumContinueSink(name, response, callback, resuri, items, xstatus, tag, 1); });
        } else {
            obj.EnumDoNext(1);
    ...
    obj.BatchEnum = function (batchname, names, callback, tag, continueOnError, pri) {
        ...
        obj.xxBatchNextEnum(batchname, Clone(names), callback, tag, {}, continueOnError, pri); obj.xxup();
    }
    ...
    obj.xxBatchNextEnum = function (batchname, names, callback, tag, results, continueOnError, pri) {
        ...
        f(n, function (stack, name, responses, status, tag0) {
            ...
            if (tag0[1].length == 0 || status == 401 || (continueOnError != true && status != 200 && status != 400)) { obj.PendingBatchOperations -= (names.length * 2); obj.xxup(); callback(obj, batchname, tag0[2], status, tag); }
    ...
    obj.xxGetMessageLog1 = function (stack, name, responses, status, tag) {
        ...
        for (i in ra) {
            ...
            if (e != null && (TimeStamp = ReadIntX(e, 0)) != -1) {
                ...
                x['Desc'] = obj.xxGetEventDetailStr(x['EventSensorType'], x['EventOffset'], x['EventData'], x['Entity']);
    ...
    obj.xxEventTrapSourceTypes = "Platform firmware (e.g. BIOS),SMI handler,ISV system management software,Alert ASIC,IPMI,BIOS vendor,System board set vendor,System integrator,Third party add-in,OSV,NIC,System management card".split(',');
    ...
    obj.xxRealmNames = ",,Redirection,PT Administration,Hardware Asset,Remote Control,Storage,Event Manager,Storage Admin,Agent Presence Local,Agent Presence Remote,Circuit Breaker,Network Time,General Information,Firmware Update,EIT,LocalUN,Endpoint Access Control,Endpoint Access Control Admin,Event Log Reader,Audit Log,ACL Realm,,,Local System".split(',');
...
// ###BEGIN###{AuditLog}
    ...
    obj.xxAmtAuditStringTable =
    {
        ...
        32: 'Screen Blanking',
        33: 'Watchdog Events',
        1600: 'Intel&reg; AMT Provisioning Started',
        1601: 'Intel&reg; AMT Provisioning Completed',
        ...
        1619: 'Intel&reg; AMT Unprovisioning Started',
    ...
    obj.GetAuditLogExtendedDataStr = function (id, data) {
        ...
        if (id == 1605) { return ["Invalid AMT access", "Invalid MEBx access"][data.charCodeAt(0)]; } // ACL Access with Invalid Credentials
        ...
        if (id == 1617) { return obj.xxRealmNames[ReadInt(data, 0)] + ", " + ["NoAuth", "Auth", "Disabled"][data.charCodeAt(4)]; } // Set Realm Authentication Mode
